...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--reference-ssm", required=True, help="NetCDF file with reference SSM on model grid.")
    parser.add_argument("--reference-var", default="reference_ssm", help="Variable name for reference SSM.")
...
def _namespace_from_kwargs(kwargs: Dict[str, object]) -> argparse.Namespace:
    ...
    missing = [
        ...
        if not hasattr(namespace, key) or getattr(namespace, key) in (None, "")
    ]
    if missing:
        raise ValueError(f"Missing required calibration inputs: {', '.join(sorted(missing))}")

    return namespace


def _get_differential_evolution():
    try:
        from scipy.optimize import differential_evolution
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "scipy is required for SWB calibration. Install scipy to run pysweb.swb.calibrate."
        ) from exc
    return differential_evolution


def calibrate_domain(**kwargs) -> None:
    args = _namespace_from_kwargs(kwargs)
    if args.workers < 1:
        raise ValueError("workers must be >= 1")

    differential_evolution = _get_differential_evolution()
    ...
    if "time" not in reference_ssm.coords:
        raise ValueError("Reference SSM data must include a 'time' coordinate.")
    ...
    if effective_precip.sizes.get("time") != reference_ssm.sizes.get("time"):
        raise ValueError("Reference SSM and forcing time dimensions do not align.")
    ...
    reference_ssm = reference_ssm.transpose("time", lat_dim, lon_dim)
    if ndvi is not None:
    ...
    if args.sm_res:
        ...
        reference_ssm = _coarsen_to_target(reference_ssm, lat_dim, lon_dim, args.sm_res)
        if ndvi is not None:
            ndvi = _coarsen_to_target(ndvi, lat_dim, lon_dim, args.sm_res)
            effective_precip, et, t, reference_ssm, ndvi = xr.align(
                effective_precip,
                et,
                t,
                reference_ssm,
    ...
    reference_vals = reference_ssm.values.astype(float, copy=False)
    ...
    print(
        f"  reference_ssm: {Path(args.reference_ssm).expanduser().resolve()} "
        f"(var={args.reference_var})",
        flush=True,
    )
