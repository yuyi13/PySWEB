...
def _require_pandas():
    if pd is None:
        raise ModuleNotFoundError("plot_time_series requires the optional dependency 'pandas'.")
    return pd


def _require_xarray():
    if xr is None:
        raise ModuleNotFoundError("plot_time_series requires the optional dependency 'xarray'.")
    return xr


def _require_matplotlib():
    if plt is None or mdates is None:
        raise ModuleNotFoundError("plot_time_series requires the optional dependency 'matplotlib'.")
    return plt, mdates
...
def _parse_timestamp(text: str) -> pd.Timestamp:
    ...
    return pandas.to_datetime(text, format="%Y-%m-%d")
...
def _filename_date_key(path: Path) -> Tuple[pd.Timestamp, pd.Timestamp]:
    ...
    if not match:
        pandas = _require_pandas()
        fallback = pandas.Timestamp(1900, 1, 1)
...
def _detect_time_dim(da: xr.DataArray) -> str:
    ...
    for dim in da.dims:
        ...
        if coord is not None and pandas.api.types.is_datetime64_any_dtype(coord.dtype):
            return dim
...
def extract_product_series(
    ...
    data = pandas.DataFrame(series_dict).sort_index()
...
def plot_time_series(
    ...
    pyplot, dates = _require_matplotlib()
    ...
    fig, axes = pyplot.subplots(len(panels), 1, figsize=figsize, sharex=True)
    if len(panels) == 1:
        axes = [axes]
...
def main():
    ...
    if args.csv_out:
        ...
        if frames:
            combined = pandas.concat(frames, axis=1).sort_index()
