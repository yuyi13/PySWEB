...
Dependencies: numpy, pandas, xarray, rioxarray, rasterio, pyproj
"""
...
def _load_soil_raster(path: Path, grid: TargetGrid, band: Optional[int] = None) -> np.ndarray:
    if not path.exists():
        raise FileNotFoundError(f"Soil raster not found: {path}")
    da = rioxarray.open_rasterio(path, masked=True)
    if band is not None:
        if "band" not in da.dims:
            raise ValueError(f"Raster {path} does not contain multiple bands.")
        if band < 1 or band > da.sizes["band"]:
            raise ValueError(
                f"Requested band {band} is out of range for {path} "
                f"(available bands: 1..{da.sizes['band']})."
            )
...
def process_soil_properties(args: argparse.Namespace, grid: TargetGrid) -> Dict[str, xr.DataArray]:
    ...
    if not soil_dir.exists():
        raise FileNotFoundError(f"MLConstraints soil directory does not exist: {soil_dir}")
    ...
    if layer_bottoms_mm.ndim != 1 or layer_bottoms_mm.size == 0:
        raise ValueError("MLConstraints layer-bottom list must be a non-empty 1-D sequence.")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--soil-mlcons-dir",
        default="/g/data/yx97/EO_collections/USYD/MLConstraints",
        help="Directory holding MLConstraints multi-band soil rasters (Clay/Sand/OC).",
    )
    parser.add_argument(
        "--soil-layer-bottoms-mm",
        nargs="+",
        type=float,
        help="Layer-bottom depths in mm for MLConstraints bands (defaults: 150 300 600 1000).",
    )

    parser.add_argument(
        "--output-dir",
        default="/g/data/ym05/sweb_model/3_sweb+mlcons_inputs",
